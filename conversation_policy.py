...
# Local model endpoint. Override with ATMAN_OLLAMA_URL; the default is only
# a loopback placeholder for local simulation, never a deployment address.
OLLAMA_URL = os.environ.get("ATMAN_OLLAMA_URL", "http://127.0.0.1:11434")
...
def audit_reply(parts, checked, intent, config=None):
    ...
    req = urllib.request.Request(setting("ollama_endpoint", OLLAMA_URL).rstrip("/") + "/api/generate",
...
def social_action(classified, speaker, raw_text="", config=None):
    ...
    # "operator" is the generic ATMAN owner role. Any other speaker label
    # passes through; deployments map their own known speakers via config.
    name = {"operator": "Operator"}.get(name.casefold(), name)
