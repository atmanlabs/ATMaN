...
def _local_root():
    ...
    return Path(root)
"""Isolated Crafter controller (example training harness): local actions, measured outcomes, bounded overnight run."""
...
# Deployment endpoints. Override with ATMAN_RELAY_URL / ATMAN_OLLAMA_URL.
# Defaults are loopback placeholders for local simulation only.
RELAY_URL = os.environ.get("ATMAN_RELAY_URL", "http://127.0.0.1:18790").rstrip("/")
OLLAMA_URL = os.environ.get("ATMAN_OLLAMA_URL", "http://127.0.0.1:11434").rstrip("/")
...
def remember(summary,episode):
 try:
  ...
  payload={'package_id':f'crafter-{RUN_ID}-{episode}','from':'atman-crafter','date':time.strftime('%Y-%m-%d'),'requested_action':'note',
   'claims':[{'content':'Crafter game experience only: '+summary,'provenance':'verified','evidence':str(OUT/'episodes.jsonl')}]}
  receipt=http(RELAY_URL+'/packages',payload,token,20)
...
def planner(controller):
 while time.time()<END and not STOP.exists():
  try:
   health=http(RELAY_URL+'/health')
   voice=EXO/'voice.activity'
   if not health.get('governor_active') and not (voice.exists() and time.time()-voice.stat().st_mtime<60):
    prompt='You are ATMAN choosing a goal in your private Crafter sandbox. Return JSON with goal (explore, gather, craft, survive) and one short reason. Only game decisions; no external actions. Current measured game state: '+json.dumps(snapshot)
    result=http(OLLAMA_URL+'/api/generate',{'model':'qwen3.5:4b','prompt':prompt,'stream':False,'think':False,'format':'json','options':{'num_predict':100,'num_ctx':2048}},timeout=60)
    ...
    if decision.get('goal') in GOALS:
     ...
     atomic(OUT/'last-atman-decision.json',{'at':time.time(),**decision})
