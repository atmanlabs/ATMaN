...
def _local_root():
    ...
    return Path(root)
def _local_root_token():
    tok_file = _local_root() / "credentials.json"
    return json.loads(tok_file.read_text())["token"]
...
for i,prompt in enumerate(PROBES):
 ...
 req=urllib.request.Request(RELAY_URL+'/chat',data=json.dumps(body).encode(),headers={'Content-Type':'application/json','Authorization':'Bearer '+token})
 start=time.time()
 try:
