"""ATMAN Situation Gym — native Windows desktop app and headless run driver."""
import json,os,sys,time,threading,queue,urllib.request,urllib.error,uuid,os
...
ROOT=Path.home()/'Documents'/'atman-training';RUNS=ROOT/'runs';ROOT.mkdir(parents=True,exist_ok=True);RUNS.mkdir(exist_ok=True)
...
def api(body):
 ...
 req=urllib.request.Request(RELAY_URL+'/chat',data=json.dumps({'text':'Scenario gym turn','source':'phone_app',**body}).encode(),headers={'Content-Type':'application/json','Authorization':'Bearer '+token})
 with urllib.request.urlopen(req,timeout=180) as r:return json.load(r)
def run(scenario,seed,notify,stop):
 ident=None;last=None
 try:
  notify('Connecting to the local ATMAN relay…')
  for attempt in range(30):
   try:
    with urllib.request.urlopen(RELAY_URL+'/health',timeout=3) as response:
     if json.load(response).get('status')=='online':break
   except OSError:pass
   if stop.wait(2):return None
  else:raise RuntimeError('ATMAN chat relay is offline. Start ATMAN, then try again.')
  ...
  notify('ATMAN is reviewing his own recorded trace. The real Judge will decide whether to store his lesson.')
...
def gui(smoke=False):
 ...
 window=tk.Tk();window.title('ATMAN Situation Gym');window.geometry('1080x780');window.minsize(850,620);window.configure(bg='#111a2b')
 ...
 heading=ttk.Frame(window,padding=20);heading.pack(fill='x');ttk.Label(heading,text='ATMAN  /  SITUATION GYM',font=('Segoe UI',22,'bold')).pack(anchor='w');ttk.Label(heading,text='Real reasoning. Simulated consequences. Evidence you can inspect.',font=('Segoe UI',11)).pack(anchor='w',pady=(5,0))
 ...
 status=tk.StringVar(value='Ready. ATMAN must already be running on this PC.');ttk.Label(gym,textvariable=status,wraplength=950).pack(anchor='w',pady=12)
 ...
 def start_run():
  ...
  stop.clear();transcript.delete('1.0','end');status.set('Running through ATMAN’s live chat pipeline…');startbutton.configure(state='disabled')
 ...
 except (OSError,ValueError):profile={'notes':'Confirmed context: Operator uses ATMAN on his PC and phone and wants better assistant behavior. No real appointments or routine errands supplied yet.','source':'conversation; schedule unconfirmed'}
