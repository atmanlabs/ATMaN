...
ROOT=_local_root();RUNS=ROOT/'runs'
...
def start(scenario,seed):
 ...
 w['goal']=("Operator: I need a USB headset for testing Atman on my PC and phone. Could you sort out a pickup around my appointment? Keep it reasonable—nothing fancy."
...
def record(ctx,cycle):
 ...
 lines=['# ATMAN Situation Gym run',f'Run: {w["id"]}',f'Scenario: {w["scenario"]}',f'Phase: {w["phase"]}',f'Process score: {w["score"]["total"]}/100','',json.dumps(w['score'],indent=2),'','Lesson: '+w.get('lesson','Pending review'),'','Memory receipt: '+json.dumps(w.get('memory_receipt',{}))]
